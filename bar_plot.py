"""
bar_plot.py
Description: 

Author: Willian T. Lunardi
Contact: wtlunar@gmail.com
License: MIT License (https://opensource.org/licenses/MIT)

Repository:
"""

import random
import threading
import time

import numpy as np
import pygame

from node import Text
from util import generate_smooth_distribution


class Bar:
    def __init__(self, x, y, ch_numb):
        self.x = x
        self.y = y
        self.red_sprite = pygame.image.load(f'sprites/bar_red.png').convert_alpha()
        self.green_sprite = pygame.image.load(f'sprites/bar_green.png').convert_alpha()
        self.shadow = pygame.image.load(f'sprites/bar_shadow2.png').convert_alpha()
        self.sprite = self.green_sprite
        self.rect = self.sprite.get_rect()
        self.ch_numb = Text(ch_numb, (x + self.rect.width // 2, self.y - 12), orientation='horizontal')
        self.value = 0
        self.target_value = 0

    def draw(self, surface, font):
        self.sprite = self.red_sprite if self.value < 0.2 else self.green_sprite
        self.rect = self.sprite.get_rect()

        # pygame.draw.circle(surface, (0, 255, 0), (self.x, self.y), 10)
        self.rect.bottomleft = (self.x, self.y + (self.rect.height * (1 - self.value)))
        surface.blit(self.sprite, self.rect)
        self.rect.bottomleft = (self.x + self.rect.width, self.y + (self.rect.height * (1 - self.value)))
        surface.blit(self.shadow, self.rect)

    def draw_ch_numb(self, surface, font, ticks=True):
        self.ch_numb.draw(surface, font)

        if ticks:
            pygame.draw.line(surface, (0, 0, 0), (self.x + self.rect.width // 2, self.y - 2), (self.x + self.rect.width // 2, self.y + 2))  # y line

    def transition_to_target_value(self, duration):
        if self.value == self.target_value:
            return

        start_value = self.value
        start_time = time.time()
        end_time = start_time + duration

        def run():
            while time.time() < end_time:
                t = (time.time() - start_time) / duration
                self.value = start_value + t * (self.target_value - start_value)
                time.sleep(0.01)

            self.value = self.target_value

        thread = threading.Thread(target=run)
        thread.start()


class BarPlot:
    def __init__(self, x, y, width, height, x_axis, bar_width=62, bottom_spacing=50, side_spacing=50, x_title='', y_title=''):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
        self.num_bars = len(x_axis)
        self.bar_width = bar_width
        self.bar_spacing = (width - side_spacing - bar_width * self.num_bars) // self.num_bars
        self.bottom_spacing = bottom_spacing
        self.side_spacing = side_spacing
        self.bars = []

        self.x_text = Text(x_title, (x + self.width // 2, self.y + self.height - (self.bottom_spacing // 2)), orientation='horizontal')
        self.y_text = Text(y_title, (x + self.side_spacing // 2, y + self.height // 2), orientation='vertical')

        pad = self.bar_spacing // 2
        for i in range(self.num_bars):
            bar_x = self.x + pad + self.side_spacing + i * (self.bar_spacing + bar_width)
            bar_y = self.y + height - self.bottom_spacing
            bar = Bar(bar_x, bar_y, ch_numb=x_axis[i])
            self.bars.append(bar)

        # Temporary
        self.last_update = time.time()
        self.update_bar_values(np.linspace(0.1, 1, self.num_bars))

    def update_bar_values(self, values, transition_duration=1):
        for i, bar in enumerate(self.bars):
            bar.target_value = values[i]
            bar.transition_to_target_value(transition_duration)

    def draw(self, surface, font):
        new_values_interval_sec = 2
        if time.time() - new_values_interval_sec > self.last_update:
            # Generate 10 random values between 0 and 1
            index = random.randint(0, self.num_bars - 1)
            values = generate_smooth_distribution(self.num_bars, index)
            self.update_bar_values(values)
            self.last_update = time.time()

        for i, bar in enumerate(self.bars):
            bar.draw(surface, font)

        # Draw bottom and side rectangle
        pygame.draw.rect(surface, (255, 255, 255), (self.x, self.y + self.height - self.bottom_spacing, self.width, self.bottom_spacing), 0)  # x axis
        pygame.draw.line(surface, (0, 0, 0), (self.x, self.y + self.height - self.bottom_spacing), (self.x + self.width, self.y + self.height - self.bottom_spacing))  # x line

        for i, bar in enumerate(self.bars):
            bar.draw_ch_numb(surface, font)

        # pygame.draw.rect(surface, (255, 255, 255), (self.x, self.y, self.side_spacing, self.height), 0)  # y axis
        pygame.draw.line(surface, (0, 0, 0), (self.x + self.side_spacing, self.y), (self.x + self.side_spacing, self.y + self.height - self.bottom_spacing))  # y line

        self.x_text.draw(surface, font)
        self.y_text.draw(surface, font)

        # pygame.draw.circle(surface, (255, 0, 0), (self.x, self.y), 10)
        pygame.draw.rect(surface, (0, 0, 0), (self.x, self.y, self.width, self.height), 1)
